...
Any combination of such pairwise orderings is allowed however the resulting lattice has to contain unqiue lables for a highest and a lowest secrecy.
...
To verify the secrecy level of an expression equals an expected secrecy level a new function application is provided.
...
----
It is an ExpressionStmt and only matches if the input_expression has the expected secrecy label during the frontend run but can be helpful if one needs to ensure the level at a point works as expected.
It is also used for testing the secrecy extension and ensuring that level evaluation for expressions works as intended. 

For the full set of secrecy type rules, see <<secrecy-rules.adoc#,the Secrecy Rules specification>>.
